...
export default function AuthLayoutBottomLink() {
    ...
    return (
        <div className='flex justify-center items-center gap-x-2 mt-4'>
            <span className={`${wix.className} text-sm md:text-lg leading-none font-medium`}>{text}</span>
            <Link href={linkAddress} className={`${wix.className} text-sm md:text-lg leading-none font-medium underline underline-offset-2 decoration-[0.5px]`}>{linkText}</Link>
        </div>
    );
}