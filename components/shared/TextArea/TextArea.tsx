...
export default function TextArea({ label, register, error }: TextAreaProps) {
    return <label className='relative pb-6 flex flex-col justify-start items-start last-of-type:mb-4'>
        ...
        <textarea className={`w-full h-[200px] p-3 rounded-lg text-brand text-xl leading-none border-[5px] ${error ? 'border-error' : 'border-transparent'} bg-transparent ${error ? 'shadow-[0_1px_5px_0_#f25137]' : 'shadow-[0_0_0_1px_#3c4959]'} focus:shadow-[0_0_0_1px_#ef820f] outline-0 md:text-2xl resize-none`} {...register}></textarea>
